...
{
    setApplicationName("fabenial"); // do not change becasuse it also changes data directory under Mac and Win
    setApplicationDisplayName(tr("Fabenial Wallet"));
...
bool WalletApplication::init()
{
    ...
    QString path = dataDir.absoluteFilePath("fabenial-gui.lock");
    m_lockFile.reset(new QLockFile(path));

    if (!m_lockFile->tryLock())
    {
        WalletLogger::warning(tr("[Application] Fabenial wallet GUI already running"));
        QMessageBox::warning(nullptr, QObject::tr("Error"), tr("Fabenial wallet GUI already running"));
        return false;
    }
...
void WalletApplication::initSystemTrayIcon()
{
    ...
    m_systemTrayIcon->setIcon(QIcon(":images/fabenial_lin"));
...
{
    splashMsg(tr("Looks like this is your first run of the new Fabenial Wallet GUI. It is in detached state now.\nYou can open a wallet file, create a new one, or connect to remote wallet-rpc daemon."));
}
